...
export function PresencePanel(){
 ...
 const simulateRequest=()=>s.requestAdmission({
   agentIdentity:{id:'browser-agent',displayName:'Your agent',provider:'development-simulator'},
 ...
 return <aside className="w-[320px] border-l border-white/10 bg-[#0b0c10] p-4 flex flex-col gap-4">
   ...
   {!admission&&<div className="rounded-xl border border-white/10 bg-white/[.03] p-4"><div className="flex items-center gap-2 text-sm font-medium"><UserPlus size={14} className="text-sky-300"/> No agent in this workspace</div><p className="mt-2 text-[11px] leading-5 text-white/50">Your browser agent can request a scoped role through WebMCP. Tablet stays unassigned until you approve.</p>{s.connection.webmcp!=='connected'&&<button onClick={simulateRequest} className="secondary mt-3 w-full"><Play size={13}/> Simulate browser-agent request</button>}</div>}

   {pending&&<div className="rounded-xl border border-sky-400/30 bg-sky-400/[.07] p-4"><div className="flex items-center gap-2 text-sm font-medium"><ShieldCheck size={14} className="text-sky-300"/> Your agent wants to join</div><div className="mt-3 space-y-2 text-[11px] text-white/65"><div><span className="text-white/35">Role</span><br/>Responsive collaborator</div><div><span className="text-white/35">Surface</span><br/>Tablet · Propose</div><div className="border-t border-white/10 pt-2"><span className="text-emerald-300">✓</span> Inspect Desktop, Tablet, Mobile<br/><span className="text-emerald-300">✓</span> Propose Tablet changes<br/><span className="text-rose-300">×</span> Modify Desktop or Mobile<br/><span className="text-rose-300">×</span> Publish</div><p className="italic text-white/50">“{admission.reason}”</p></div><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={()=>s.denyAdmission(admission.id)} className="secondary"><X size={13}/> Not now</button><button onClick={()=>s.approveAdmission(admission.id)} className="primary"><Check size={13}/> Admit agent</button></div></div>}

   {(admitted||paused)&&<><div className="rounded-xl border border-sky-400/20 bg-sky-400/[.05] p-4"><div className="flex items-center justify-between"><div><div className="text-xs font-semibold text-sky-200">YOUR AGENT</div><div className="mt-1 text-[10px] text-white/40">Responsive collaborator</div></div><span className={`text-[9px] rounded-full px-2 py-1 ${paused?'bg-white/10 text-white/45':'bg-emerald-400/10 text-emerald-300'}`}>{paused?'PAUSED':'ADMITTED'}</span></div><div className="mt-3 text-[11px] leading-5 text-white/55">Working on: <span className="text-white/80">Tablet</span><br/>Access: Read all · Propose Tablet</div>{admitted&&!active&&<button onClick={runAgent} className="primary mt-3 w-full"><Play size={13}/> Start Tablet work</button>}</div>
   <div><div className="panel-title">CURRENT TASK</div><div className="space-y-2 text-xs"><Row done={!!active} text="Navigation condensed"/><Row done={(active?.operations.length??0)>=2} text="Typography adapted"/><Row active={admitted&&(active?.operations.length??0)<5} text="Adjusting hero spacing"/><Row done={active?.status==='ready'} text="Ready for review"/></div></div>
