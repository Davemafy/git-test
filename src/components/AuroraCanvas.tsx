...
export function AuroraCanvas({breakpoint,project}:{breakpoint:Breakpoint;project:Project}){
 ...
 return <section className={`relative ${size[breakpoint]} flex-1 overflow-hidden rounded-xl border bg-[#111218] shadow-2xl shadow-black/30 ${proposal?'border-sky-500/55':'border-white/10'}`}>
  <div className="flex h-9 items-center justify-between border-b border-white/10 px-3 text-[10px] tracking-[.13em] text-white/50"><span>{labels[breakpoint]}</span><motion.span layout className={ownerLabel==='YOUR AGENT'?'text-sky-300':ownerLabel==='YOU'?'text-violet-300':''}>{ownerLabel}</motion.span></div>
  <div className="relative h-[600px] select-none overflow-hidden bg-[#f2f0e9] text-[#111]">
   ...
   <div onClick={()=>choose('hero')} style={{padding:hero.padding??24,gap:hero.gap??22}} className={`${splitHero?'grid grid-cols-2':'flex flex-col'} min-h-[285px] items-center ${selected==='hero'?'ring-2 ring-inset ring-violet-500':''}`}>
    ...
    <motion.div layout onClick={event=>{event.stopPropagation();choose('heroMedia')}} style={{order:media.order??2}} className={`w-full rounded-2xl border border-black/10 bg-black p-2 ${selected==='heroMedia'?'outline outline-2 outline-violet-500':''}`}><div className={`rounded-xl bg-gradient-to-br from-violet-500 via-indigo-400 to-cyan-300 p-3 ${media.imageRatio==='16:9'?'aspect-video':media.imageRatio==='1:1'?'aspect-square':'aspect-[4/3]'}`}><div className="h-full rounded-lg bg-black/75 p-3 text-white"><div className="mb-3 flex gap-1"><span className="h-1.5 w-1.5 rounded-full bg-white/30"/><span className="h-1.5 w-1.5 rounded-full bg-white/30"/></div><div className="mb-2 h-2 w-3/4 rounded bg-white/20"/><div className="grid h-2/3 grid-cols-2 gap-2"><div className="rounded bg-white/10"/><div className="rounded bg-white/10"/></div></div></div></div></motion.div>
   </div>
   ...
   {showAgent&&<motion.div initial={{opacity:0,scale:.7,x:280,y:24}} animate={{opacity:1,scale:1,x:cursor.x,y:cursor.y}} transition={{type:'spring',stiffness:320,damping:28}} className="pointer-events-none absolute left-0 top-0 z-20"><svg width="18" height="22" viewBox="0 0 18 22" fill="none" className="drop-shadow"><path d="M2 1.5L16 12.5L9.3 13.2L6.1 20.1L2 1.5Z" fill="#0EA5E9" stroke="white" strokeWidth="1.2"/></svg><div className="ml-3 -mt-1 whitespace-nowrap rounded-md bg-sky-500 px-2 py-1 text-[9px] font-semibold text-white shadow-lg">YOUR AGENT · {project.nodes[latestAgentTarget]?.name??'Tablet'}</div></motion.div>}
   {showHuman&&<motion.div initial={{opacity:0,scale:.9}} animate={{opacity:1,scale:1}} className="pointer-events-none absolute left-8 top-60 z-20"><svg width="18" height="22" viewBox="0 0 18 22" fill="none"><path d="M2 1.5L16 12.5L9.3 13.2L6.1 20.1L2 1.5Z" fill="#8B5CF6" stroke="white" strokeWidth="1.2"/></svg><div className="ml-3 -mt-1 rounded-md bg-violet-500 px-2 py-1 text-[9px] font-semibold text-white shadow-lg">YOU</div></motion.div>}
