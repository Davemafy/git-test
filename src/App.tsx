...
export default function App(){
 ...
 useLayoutEffect(()=>{
  ...
  usePresenceStore.setState(state=>({
   admissions:[],
   participants:state.participants.filter(p=>p.type!=='agent'),
   assignments:state.assignments.filter(a=>a.participantId!=='agent'),
   connection:{...state.connection,agent:'absent'},
  }))
 },[])
 useEffect(()=>{queueMicrotask(registerWebMcp)},[])
 const completed=s.proposals.some(p=>p.status==='accepted')
 const agentLabel=s.connection.agent==='working'?'Your agent is on Tablet':s.connection.agent==='pending'?'Agent requesting access':s.connection.agent==='paused'?'Agent paused':s.connection.agent==='revoked'?'Agent removed':'No agent admitted'
