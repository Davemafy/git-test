...
export default function App(){
 ...
 useEffect(()=>{if(new URLSearchParams(location.search).get('demo')==='1')s.resetDemo(); queueMicrotask(registerWebMcp)},[])
 ...
 return <main className="h-screen min-h-[720px] bg-[#090a0d] text-white flex flex-col overflow-hidden">
   <header className="h-14 border-b border-white/10 px-4 flex items-center justify-between shrink-0 bg-[#0c0d11]">
    ...
    <div className="flex items-center gap-2"><div className="hidden lg:flex items-center gap-2 mr-2 rounded-full px-3 h-8 border border-white/10 text-[10px] text-white/55"><span className={`w-1.5 h-1.5 rounded-full ${s.connection.agent==='working'?'bg-emerald-400':'bg-white/25'}`}/>{s.connection.agent==='working'?'Agent is working on Tablet':'Agent ready'}</div><button className="icon" aria-label="Undo" onClick={()=>s.undo()}><RotateCcw size={15}/></button><button className="icon" aria-label="Redo" onClick={()=>s.redo()}><RotateCw size={15}/></button><button className="icon" aria-label="Developer inspector" onClick={()=>s.setDevOpen(!s.devOpen)}><Bug size={15}/></button><button className="secondary hidden sm:flex"><Share2 size={13}/> Share</button><button className="primary">Publish</button></div>
   </header>
   ...
   <div className="min-h-0 flex-1 flex">
    <section className="min-w-0 flex-1 flex flex-col bg-[#0d0e12]">
     {s.staleBanner&&<div className="h-10 px-4 flex items-center justify-between bg-amber-400/10 border-b border-amber-400/20 text-[11px] text-amber-100"><span>Your layout changed while the agent was working. It’s re-reading the latest state.</span><button onClick={s.clearStaleBanner}>Dismiss</button></div>}
     <div className="min-h-0 flex-1 overflow-auto p-4"><div className="flex gap-4 min-w-[970px] h-full"><AuroraCanvas breakpoint="desktop" project={s.project}/><AuroraCanvas breakpoint="tablet" project={s.project}/><AuroraCanvas breakpoint="mobile" project={s.project}/></div></div>
     <div className="h-11 border-t border-white/10 px-4 flex items-center gap-4 bg-[#0b0c10] text-[10px] text-white/35 shrink-0"><span>Desktop</span><span>Tablet</span><span>Mobile</span><span className="ml-auto flex items-center gap-2">{completed&&<><CheckCircle2 size={12} className="text-emerald-400"/> Tablet changes accepted</>}<span>Autosaved</span><span>·</span><span>r{s.canonicalRevision}</span><button onClick={s.resetDemo} className="ml-2 text-white/55 hover:text-white">Reset Aurora Demo</button></span></div>
    </section>
    <Inspector/><PresencePanel/>
   </div>
