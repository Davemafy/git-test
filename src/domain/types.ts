...
export type ErrorCode = 'STALE_STATE'|'PERMISSION_DENIED'|'CONSTRAINT_VIOLATION'|'INVALID_OPERATION'|'SURFACE_NOT_OWNED'|'NOT_FOUND'
...
export interface Participant { id:string; name:string; type:ParticipantType; color:'violet'|'blue'|'neutral'; status:'idle'|'working'|'paused'; breakpoint?:Breakpoint }
...
export interface Proposal { id:string; participantId:string; breakpoint:Breakpoint; baseRevision:number; operations:OperationChange[]; status:ProposalStatus; createdAt:number; explanation:string; affectedComponentIds:string[] }
