...
export type AdmissionStatus = 'discovered' | 'requesting' | 'pending_user_approval' | 'admitted' | 'paused' | 'revoked' | 'expired'
...
export interface ComponentProps { text?:string; label?:string; src?:string; href?:string; role?:string; minTouchTarget?:number }
export interface ComponentNode { id:string; type:'page'|'section'|'container'|'nav'|'text'|'image'|'button'|'grid'|'footer'; name:string; props:ComponentProps; responsive:Partial<Record<Breakpoint,ResponsiveProps>>; children:string[] }
...
export interface AgentAdmission { id:string; agentIdentity:AgentIdentity; status:AdmissionStatus; requestedRoleId?:string; requestedScopes:CapabilityScope[]; grantedScopes:CapabilityScope[]; reason?:string; sessionId:string; createdAt:number; admittedAt?:number; expiresAt?:number }
